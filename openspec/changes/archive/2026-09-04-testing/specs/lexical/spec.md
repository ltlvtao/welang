...
#### Scenario: The testing keywords are reserved

- **WHEN** `test` or `mock` appears where an identifier is required (for example a variable named `test`)
- **THEN** the parser reports a syntax diagnostic naming the reserved word; the two are keywords under the testing chapter's amendment
