...
#### Scenario: A test body is not effect-checked

- **WHEN** a test block's body calls a function declaring `effect io` and the test block declares nothing
- **THEN** no effect diagnostic fires — the test block has no segment to check against, the testing chapter's amendment; the called declaration's own segment is where the effect truth stands
