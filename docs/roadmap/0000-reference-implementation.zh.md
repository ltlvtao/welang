...
| B2b | `stdlib-collections` | B2 余下部分按实现落地：集合面背后的真数据结构——List 载体重建为稳定 32 字节句柄（push 就地换数据指针，别名因此看得见增长——第 17 章的两句柄一对象——零 IR 漂移）、Map/Set 为开放寻址表（线性探测、墓碑删除、倍增 rehash）、Eq 键域上的 `mapOf`/`setOf` 构造（String 键按内容 FNV-1a 哈希与比较、两表长度不匹配即任务失败）、十三员变异成员面检查并发射（List `add`/`removeAt`/`get`/`size`，Map `put`/`get`/`remove`/`keys`/`size`，Set `add`/`remove`/`has`/`size`，`get` 以 `Option` 作答、越界即 `None`）、Map/Set 以单指针字过签名且收集器描记两区、泛型 `mapOf` 落 E1804 mock 类目、Builder 经裁定拒收；直接 Map/Set 迭代维持申报非目标——受认可习惯形为 `keys()` + `get()`——登记为 follow-up #26；conformance 893 → 919、codegen 顶层单测 461 → 474；残余停面披露于 `docs/benchmarks.md` | done 2026-09-16 |
| B3a | `stdlib-conversions` | 自举轨第一段：`std.string` 的七枚转换原语——`parseInt`/`parseUInt`/`parseFloat` 收十进制核心以 `Option` 作答（畸形串、内嵌 NUL、越界值皆 `None`；`2.5e-320` 以 subnormal 存活、`1.0e999` 不存活），`runeCode`/`runeFrom`/`floatBits`/`floatFromBits` 为全函数过桥（rune 对码点恒等、float 对位型重解释、双回环成立）——经键控槽走 C 族、mock 面骑同一槽，两枚先决缺口随线闭合（string 模块乘测试塔程序面、`Rune` 过 fn 签名）；conformance 919 → 933、codegen 顶层单测 474 → 482，运行面披露于 `docs/benchmarks.md`，登记 follow-up #27 与 #28 | done 2026-09-17 |
| B3b | `bootstrap` | wec 项目居仓库顶层 + 词法器移植 | pending |
| B3c | `bootstrap` | parser | pending |
| B3d | `bootstrap` | checker（实测过大时现场拆分） | pending |
| B3e | `bootstrap` | codegen（实测过大时现场拆分；逐字节对账为其差分杠） | pending |
| B3f | `bootstrap` | cli/test/fmt/vet/doc 面 + 三段式闭环——Go 宿主构建编译 We 编译器、其产物再编译编译器一次、两段产物逐字节一致——与 conformance 套件第二面（We 宿主编译器下全绿） | pending |
...
25. **消费 `timeout` scope 所答的 `Result`。** 规范给 `timeout` 各形一枚 `Result`——`Ok(b)`/`Err(TimedOut)`（`docs/spec/1800-concurrency.md:231`）——而检查器接受它的消费者：`scope timeout(1000) { 7 }` 之上的 `match r { Ok(v) => … }` 过检查（exit 0），构建却停 `bndMainBody`（exit 70）。发射侧交给绑定的是该 scope 构造的三字 `Ok/Err` 和式——与 `?` 经 `emitQuestion` 的 timeout 臂所消费的是同一组字——但从绑定里把这组字读回来的臂没有发射，故规范许给 timeout scope 的值除 `?` 外没有读路。相邻而**不属**此缺口：拿该 `Result` 与其载荷相比（同一 scope 之后 `if r == 7`）报的 `E0501` 是检查器判对——同一节规范把普通与 `collectAll` 的值定为体自身的块值、唯 `timeout` 各形是 `Result`，故永不插入胁迫——黄金 `check-e0501-scope-timeout-value` 钉住该面。B1b codegen-full（T13，阻塞前置探针 q2，2026-09-15）发现。
26. **Map 与 Set 的迭代发射。** 第 17 章的要求使 `Map`/`Set` 实现 `Iterable`，检查器亦照此办理：map 或 set 值上的 `for` 过检查（黄金 `build-bnd-map-iteration` 钉住 `for (k, v) in m` 到达构建、exit 70）。发射塔停在那里，规范批准的迭代在参考构建中没有可跑面——建集合的变更（B2b）把直接发射的打开申报为非目标，正典习惯形是 `let ks = m.keys(); for k in ks` 加 `m.get(k)`（端到端可跑，快照序 = 单次 `keys()` 调用内的表序）。打开它的变更须选定发射形（对表活槽的快照迭代器——镜像 List 载体 `__we_list_snap` 姿态——或协议自身的 `iterator`/`next` 对），并裁定黄金所携的 `(k, v)` 元组形是否唯一拼写。B2b stdlib-collections（proposal 申报非目标，归档时登记）发现。
27. **`\u` 逃逸与 rune 字面量的码点域无校验。** 今日 Rune 是 i64 码点、无范围检查：`\u{…}` 逃逸累加一至六枚十六进制位即止（无 0x10FFFF 上限、无代理区排除，`codegen.go:16718-16742`），rune 字面量收任意单字符。`runeFrom` 是该事实之上的忠实恒等过桥——不新增校验也不假装有校验，rune 对收进 stdlib 面未改该域。第 1 章与第 7 章应否为逃逸与字面量固定码点域（届时 `runeFrom` 在边界上欠什么）由修订裁定。B3a stdlib-conversions（design D5 披露，归档时登记；非 B3a 改动）发现。
28. **We 宿主编译器下的依赖解析。** B3 裁决（2026-09-16 第 4 条）把依赖解析申报为自举轨 We 宿主二进制的非目标：`[deps]` 表解析保留，真声明依赖的项目诚实停（报未实现面并退出，不解析任何东西）。Go 宿主工具链的第 22 章表面（M13）不动。B3a stdlib-conversions（轨道裁决，归档时登记）发现。
