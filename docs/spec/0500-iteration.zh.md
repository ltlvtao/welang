...
for 语句是 `for name in expr block`：`for` 与 `in` 是第 1 章关键字，`name` 是标识符、`_`（绑定空）或第 8 章的元组模式，`expr` 是表达式，体是第 2 章块。头中的元组模式按第 8 章规则逐元素绑定：其绑定匹配元素类型的元组，对非元组元素类型的模式依第 7 章在绑定位置以 `E0501` 拒绝。for 是语句：不产出值，MUST NOT 出现在要求值的位置（`E0202`）。可迭代表达式在迭代开始前恰求值一次。语句按序迭代可迭代值的元素，每个恰一次；每个元素使体执行一次且 `name` 绑定到该元素，每次迭代的绑定相互独立、作用域限于该次执行——MUST NOT 泄漏过循环。体内 `break` 与 `continue` 依第 3 章 Break and continue 合法：for 对该条而言是循环。可迭代表达式的类型 MUST 为其元素类型 `T` 实现第 11 章可迭代协议的 `Iterable<T>`——拒绝码为 `E0901`——语句的执行模型，一个迭代器经 `next` 推进直至 `None`，是第 11 章的 for 循环协议。
...
- **WHEN** 出现 `for (a, b) in pairs { ... }` 且可迭代值的元素类型是元组
- **THEN** 模式依第 8 章逐元素绑定，每元素一组绑定，作用域同任何循环绑定

### Requirement: 区间表达式

区间运算符 `..` 是二元运算符，第 2 章优先级表最松的一档，本档非结合：`a..b..c` 以 `E0104` 拒绝。区间右排他：`start..end` 不含 `end`。整数操作数下——第 7 章的八种整数类型；任何其他类型的操作数，浮点在内，依第 11 章以 `E0902` 拒绝——迭代按单位步进从 `start`（含）到 `end`（不含）依次产出每个值——`0..3` 迭代 `0`、`1`、`2`——start 不低于 end 的区间零元素。区间是表达式：表达式合法处皆可用，不限于 for 头；任一操作数上的算术与后缀形式结合更紧（`0..n-1` 即 `0..(n-1)`）。Range 类型、其类型化与其内建 `Iterable` 实现归第 11 章。
...
for (a, b) in pairs {       // 头中的元组模式：依第 8 章
    use(a, b)               // 逐元素绑定
}
```
