...
- **WHEN** 出现 `interface Source<T> { fn pick<U>(mut self, f: fn(T) -> U) -> Option<U> }`
- **THEN** 方法的子句依"泛型参数声明"在名字后解析成立，作用域仅该方法并在每个调用处定出；集合章节的迭代器组合子是首批声明载体
...
#### Scenario: 方法子句遮蔽接口参数被拒绝

- **WHEN** 写出 `interface Sink<T> { fn send<T>(mut self, item: T) }`——方法子句重声明接口的参数名
- **THEN** 编译器以 `E0826:` generic parameter shadows an enclosing parameter 拒绝；规则在此批准的方法子句嵌套中现行约束
