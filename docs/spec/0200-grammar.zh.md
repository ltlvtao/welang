...
语句边界是推断的，不是书写的；源码不携带分号。圆括号 `( ... )`、方括号（含属性单元 `#[ ... ]`）与 record 构造表达式的花括号内部，换行不承载意义。界定块的花括号不是本条意义上的括号，其内适用深度 0 规则。在括号深度 0，每个源行最后一个 token 之后存在语句边界，UNLESS 该 token 属于续行集：某个二元运算符（`+ - * / % == != < <= > >= && || & | ^ << >> ..`）、`=` 或 `.`。续行集是闭集；扩展它是 spec 变更。能起始语句的 token 是固定类：标识符、字面量、关键字、`(`、`{` 与前缀运算符 `!`、`-`、`~`。
...
#### Scenario: 构造花括号内换行无意义

- **WHEN** record 构造跨行，例如相继各行 `User {`、`id: UserId(1),`、`name: "Ada",`、`}`
- **THEN** 换行不起作用；构造的花括号是括号，字段以逗号分隔，其内不推断语句边界
...
语句形式按语句族逐族批准，每族由其归属章节批准；各族及其成员的集合按章节封闭，只能经归属章节的 spec 层变更增长。本章批准：绑定语句（`let name = expr` 与 `var name = expr`，各自可在 `=` 前带类型注解 `name: type`；名字位接受标识符，或依第 8 章接受逐元素解构的元组模式；类型的文法由类型章节批准）、赋值语句（`name = expr`，更丰富的赋值目标随其归属章节批准——第 8 章对直接目标关死 record 字段一侧，第 10 章恰开一目字段目标形式：mut self 方法体内的 `self.field = expr`；其余一切字段目标不合任何产生式）、表达式语句（任意表达式作为一项）。控制流章节批准控制流语句（if、while、loop、break、continue、return、defer）；迭代章节批准 for 语句；资源章节批准 scope resource 语句。赋值是语句不是表达式：它不产出值，MUST NOT 出现在要求表达式的位置。
...
- **WHEN** `let (a, b) = pair` 作为一项出现
- **THEN** 它是名字位持第 8 章元组模式的绑定语句；各元素逐元素绑定
...
- **WHEN** `u.name = "bob"` 出现，`u` 是 record 值
- **THEN** 它不合任何已批准赋值目标——唯一的字段目标形式是第 10 章方法接收者的 `self.field`，而 `u` 不是接收者——MUST 按第 2 章意外 token 诊断（`E0105`）拒绝；方法之外的修改是第 8 章的更新表达式
...
- **WHEN** 解析 `f()?.name`
- **THEN** 分组为 `(f()?).name`——传播后缀是本骨架的后缀，由第 14 章授予，成员访问落在解包后的载荷上
...
- **WHEN** `scope timeout(500) { ... }`、`task effect net { ... }` 或 `select { ... }` 表达式出现在表达式位置
- **THEN** 该形式遵循并发章的 Requirements；本骨架的 primary、postfix、unary 层不变
...
表是闭集：加运算符、改档或改结合性都是 spec 变更。`=`、`.`、`->`、`=>` 不是二元运算符，在本章范围内绝不出现在表达式内部。`?` 是第 14 章授予的档 1 后缀，不是二元运算符，经该章修订入表。
