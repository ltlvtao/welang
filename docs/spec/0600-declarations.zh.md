...
#### Scenario: test 块是测试模块的顶层项

- **WHEN** 文件 `math_test.we` 在其 import 与 fn 声明之间持有 `test "adds" { let _ = add(1, 2) }`，而一个不以 `_test.we` 结尾的文件持有同一块
- **THEN** 前者依第 20 章解析为一个顶层项；后者在那里被拒（`E1801`）——`_test.we` 名字是承载该合法性的模块同一性事实
