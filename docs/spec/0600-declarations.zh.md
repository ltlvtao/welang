...
源文件即模块。文件顶层是顶层项的序列：import 声明、fn 声明、顶层 let 绑定。各项次序任意；排序约定是格式化器的事，不是文法的事。语句与表达式 MUST NOT 作为顶层项出现——它们只存在于块内——不适配任何项产生式的顶层 token 序列 MUST 以第 2 章意外 token 诊断（`E0105`）拒绝。模块名是按第 1 章命名约定指名它的点分小写路径；路径如何解析到文件（目录映射、标准库优先、依赖缓存）由模块系统章节批准。

#### Scenario: 一个文件解析为一个模块

- **WHEN** 源文件持有 import、fn 声明与顶层 let 绑定
