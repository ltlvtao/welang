...
类型引用——填充第 2 章与第 6 章批准的每个类型注解槽的形式——是命名类型：PascalCase 标识符，可选模块限定为 `module.Name`，按第 6 章触达已导入模块的公开类型。元组类型、函数类型、泛型应用与其他一切类型文法尚不存在；各自随其归属章节经本章修订到达。类型槽持有命名引用以外的任何东西 MUST 以第 2 章意外 token 诊断（`E0105`）拒绝。良构名字是否解析到已批准类型，随模块解析由模块系统章节决定。
...
- **WHEN** 类型槽持有元组或函数类型文法，例如 `let pair: (Int64, Int64) = ...`
...
有值块的类型是其末项表达式条目的类型；无值块——末项为语句或空块——没有类型。这落地第 2 章延后的块值类型化；无值块在要求类型处如何表现，随 unit 类型由复合类型章节批准。
...
- **WHEN** 块的末项是语句，或块为空
- **THEN** 块没有值也没有类型；它在要求类型处的使用随复合类型章节
