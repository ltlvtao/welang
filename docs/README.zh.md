...
- **设计语料用位置编号。** `docs/spec/` 下的正典规范章节文件为 `<nnnn>-<slug>.md`——四位零填充、步长 100（`0000`、`0100`、`0200`……），新章节可在相邻编号间插入而无需重排。`0000` 保留给规范第 0 章（基础与原则）。ADR 使用同样步长：`ADR-<nnnn>-<slug>.md`。翻译文件继承全部命名元素（见"文档语言"）。
...
- **固定结构名豁免。** `README.md`、`AGENTS.md`、`SKILL.md`、`change.yaml` 与四件套工件文件名永不带前缀——工具与跨工具约定依赖这些名字。
